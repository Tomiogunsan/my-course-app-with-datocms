...
export default function PricingCard({details}) {
  if (details.featured){
  return (
    <div className='w-full bg-white border-yellow-400 max-w-sm rounded-lg p-6 border-4'>
       ...
       <p className='text-purple-500 text-6xl font-bold mb-10'>
        ...
        <ul>
          ...
          <li key={index} className='text-purple-500 my-2 text-sm bg-yellow-400' >
            {bullet}
          </li>)}
        </ul>
        <button className='mx-auto w-full block bg-yellow-400
        rounded-md text-purple-500 mt-6 mb-4 px-4 py-2'>{details.buttonText}</button>
         <small className='text-purple-500 block text-center
        text-xm'>{details.finePrint}</small>
    </div>
  )
  }else {
    return(
    <div className='w-full bg-purple-500 max-w-sm rounded-lg p-6'>
       ...
       <p className='text-white text-6xl font-bold mb-10'>
        ...
        rounded-md text-white mt-6 mb-4 px-4 py-2'>{details.buttonText}</button>
         <small className='text-purple-200 block text-center
        text-xm'>{details.finePrint}</small>
    </div>
    )
  }
